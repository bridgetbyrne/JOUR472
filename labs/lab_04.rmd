...
media_expenses <- read_csv("data/maryland_expenses.csv") |> clean_names() |>
  filter(expense_category == 'Media')|>
  filter(!(payee_name == 'NA')) 

 
write_csv(as.data.frame(media_expenses),"data/media_expenses.csv") 

cleanmedia_expenses <- read_csv("data/cleanmedia_expenses.csv") 
  

totalfacebookmedia_expenses <- read_csv("data/cleanmedia_expenses.csv") |>
...
**A2.** The top ten spenders are all political action committees. There are a few PACs for house candidates and at least one political party PAC. 
...
**A3.** 
There are no committees that seem to represent Cox or Schifanelli, who was the winner of the republican primary. That seems like it's either important or I made a mistake. I'm not sure how a  candidate could win without social media ads. 
The biggest recipients of media expenses from the Moore campaign was to "Moore Miller For Maryland Slate" and in the "Transfers Out to Other Maryland Treasurers" chatagory. 
...
**A4.** 
the top five total spenders are Moore Miller For Maryland Slate (6,078,481.01), Blair  David for Montgomery (3,121,373.14), Lierman  Brooke for Maryland (2,706,907.39), Adams  Tim(Timothy) Friends of (2404310.34), and Franchot Anderson-Walker for Maryland Slate (2,292,129.04).Franchot Anderson-Walker for Maryland Slate is the only committee to be in top five spending for both lists. Out of the top five from question one, it does not seem like any of them spent all their money on facebook.

 

```{r}

totalmedia_expenses <- read_csv("data/cleanmedia_expenses.csv") |>
group_by(committee_name) |>
  summarise(total_spending = sum(amount)) 
...
**A5**  Why did Franchot spend over 108,882 between his on facebook ads? I that's almost 45% of his total media spending. Maybe it would only be a story for political science geeks, but I think there could be a story there. 
57928.25+50953.81 
122601.6+122601.6 
108882.1/245203.2

I don't understand why the both governor candidates did not spend any media money on facebook. That doesn't seem right to me because social media ads are very important and because Franchot spent so much money on facebook. 
