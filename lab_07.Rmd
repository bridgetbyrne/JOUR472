...
```{r echo=FALSE}
 ...
 census_api_key("6d88938491f9e12ead2aa9c0d276b3973458ffb4", install = TRUE)
...
```{r}
overdose_data <- read_csv("data/montgomery_2022_overdoses.csv") |>
 mutate(zip = as.character(zip))
 
overdose_data_zip <- montgomery_2022_overdose |>
  group_by(zip)|>
...
overdose_data_zip_18 <- overdose_data_zip_18|> inner_join(md, by=c('zip'=GEOID))|>
rename(over_18_population=estimate) |> 
mutate(rate_of_calls_per_100=(count_calls/over_18_population)*1000) |> 
aragnge(rate_of_calls_per_1000)
...
overdose_data_zip_18 <- overdose_data_zip_18 |>
  mutate(
    Min_Population = estimate - marginoferror,
    Max_Population = estimate + marginoferror
  )

overdose_data_zip_18 <- overdose_data_zip_18|>
  mutate(
    Calls_Per_1000_Population = (total_calls / estimate) * 1000
  )
