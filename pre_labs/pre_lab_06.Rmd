...
no, we had to fix 2022 because it had one different formatted term, "Total" instead of "TOTAL"and we did that with "rename" 
...
It says there are 24 rows and that makes sense. There are 24 rows in both data frames so when joined the number of rows should not change. 
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** Population of people over 18 and by race, per county  
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince Georges county has the greatest percentage of 18+ black population, at around 61% and PG county is 77% percent of the county is democratic.
...
Howard have the highest population, by far. Montgomery has 15% and Howard has 19% and the next down, is Baltimore County at 6%
```{r}
maryland_population_with_voters_and_demographics |>
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer** Change from numeric to character. If you're changing the data type of something give it the same name. 
